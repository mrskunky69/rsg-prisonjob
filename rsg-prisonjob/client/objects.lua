...
-- Functions
local function GetClosestprisonObject()
...
-- Events
RegisterNetEvent('prison:client:spawnPObj', function(item)
    RSGCore.Functions.Progressbar("spawn_object", Lang:t("progressbar.place_object"), 2500, false, true, {
        ...
        TriggerServerEvent("prison:server:spawnObject", item)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
...
RegisterNetEvent('prison:client:deleteObject', function()
    local objectId, dist = GetClosestprisonObject()
    if dist < 5.0 then
        RSGCore.Functions.Progressbar("remove_object", Lang:t('progressbar.remove_object'), 2500, false, true, {
            ...
            TriggerServerEvent("prison:server:deleteObject", objectId)
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "weapons@first_person@aim_rng@generic@projectile@thermal_charge@", "plant_floor", 1.0)
            RSGCore.Functions.Notify(Lang:t("error.canceled"), 'error')
        end)
...
RegisterNetEvent('prison:client:removeObject', function(objectId)
    NetworkRequestControlOfEntity(ObjectList[objectId].object)
    DeleteObject(ObjectList[objectId].object)
    ObjectList[objectId] = nil
end)

RegisterNetEvent('prison:client:spawnObject', function(objectId, type, player)
...
RegisterNetEvent('prison:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < Config.MaxSpikes then
        if PlayerJob.name == "prisonguard" and PlayerJob.onduty then
            ...
            TriggerServerEvent('prison:server:SyncSpikes', SpawnedSpikes)
        end
    else
...
RegisterNetEvent('prison:client:SyncSpikes', function(table)
    SpawnedSpikes = table
end)
