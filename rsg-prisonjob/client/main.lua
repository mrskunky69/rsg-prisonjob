...
AddEventHandler('RSGCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and PlayerJob.name ~= "prisonguard" then
        if DutyBlips then
...
RegisterNetEvent('RSGCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name == "prisonguard" and PlayerJob.name ~= "prisonguard" then
        CreatePrompts()
    ...
    if JobInfo.name ~= "prisonguard" then
        if DutyBlips then
            for k, v in pairs(DutyBlips) do
                RemoveBlip(v)
            end
...
RegisterNetEvent('prison:client:sendBillingMail', function(amount)
...
RegisterNetEvent('prison:client:UpdateBlips', function(players)
    if Config.ShowBlips then
        if PlayerJob and (PlayerJob.name == 'prisonguard' or PlayerJob.name == 'ambulance') and onDuty then
            if DutyBlips then
                for k, v in pairs(DutyBlips) do
                    RemoveBlip(v)
                end
...
RegisterNetEvent('prison:client:SendprisonEmergencyAlert', function()
    local Player = RSGCore.Functions.GetPlayerData()
    TriggerServerEvent('prison:server:prisonAlert', Lang:t('info.officer_down', {lastname = Player.charinfo.lastname, callsign = Player.metadata.callsign}))
